...
function mapOverStylesAndTransformBackgroundImageDeclarations(styles, includeMode, filter,cb) {
	// Map over all
	return mapOverStylesAndTransformAllBackgroundImageDeclarations(styles, filter, function(declaration) {
...
function mapOverStylesAndTransformAllBackgroundImageDeclarations(styles, filter, cb) {
	...
	function transformDeclaration(declaration, declarationIndex, declarations) {
		...
		const regMatch = new RegExp(filter.pattern, filter.attributes);
        function replace () {
            transformedDeclaration.value = transformedDeclaration.value.replace(regMatch,'');
        }

        function needFilter () {
        	if(filter){
				return regMatch.test(transformedDeclaration.value)
            }else {
        		return true;
			}
        }
		...
		if(transformedDeclaration.property === 'background-image' && needFilter() ) {
            replace();
			return cb(transformedDeclaration, declarationIndex, declarations);
		}
		// Background is a shorthand property so make sure `url()` is in there
		else if(transformedDeclaration.property === 'background' && needFilter()) {
            replace();
